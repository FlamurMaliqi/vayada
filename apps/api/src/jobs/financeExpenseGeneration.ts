...
// prettier-ignore
type Handled = { outcome: "succeeded" | "replayed" } | { outcome: "incomplete"; code: string } | { outcome: "continuation"; code: "bounded_sweep_remaining"; blockedRuleIds: string[]; incompleteCode?: string } | { outcome: "failed"; code: string; retryable: boolean };
...
async function invoke(client:pg.PoolClient,job:Job,attemptId:string,now:Date):Promise<Handled>{
  ...
  if(!result.ok)return failure(result.code);if(result.outcome==="missing_evidence")return {outcome:"incomplete",code:result.code};return {outcome:result.outcome==="replayed"?"replayed":"succeeded"};
}
...
async function succeed(client:pg.PoolClient,job:Job,attemptId:string,attempt:number,handled:Exclude<Handled,{outcome:"failed"|"continuation"}>,now:Date):Promise<Counter>{
  ...
  await client.query("UPDATE platform.jobs SET status='succeeded',finished_at=$3,locked_at=NULL,locked_by=NULL,updated_at=$3,job_metadata=(job_metadata-'pendingIncompleteCode'-'pendingNonActionableRuleIds'-'lastErrorCode')||jsonb_build_object('outcome',$4::text,'outcomeCode',$5::text) WHERE id=$1::uuid AND attempts_count=$2 AND status='running'",[job.id,attempt,now.toISOString(),handled.outcome,code]);
...
async function continueSweep(client:pg.PoolClient,job:Job,attemptId:string,attempt:number,handled:Extract<Handled,{outcome:"continuation"}>,now:Date):Promise<RunOutcome>{
  await client.query("UPDATE platform.job_attempts SET status='succeeded',finished_at=$4,error_metadata=jsonb_build_object('outcome','continuation','outcomeCode','bounded_sweep_remaining') WHERE id=$1::uuid AND job_id=$2::uuid AND attempt_number=$3",[attemptId,job.id,attempt,now.toISOString()]);
  await client.query("UPDATE platform.jobs SET status='pending',run_after=$3,finished_at=NULL,locked_at=NULL,locked_by=NULL,max_attempts=max_attempts+1,updated_at=$3,job_metadata=job_metadata||jsonb_strip_nulls(jsonb_build_object('outcome','continuation','outcomeCode','bounded_sweep_remaining','pendingIncompleteCode',$4::text,'pendingNonActionableRuleIds',$5::jsonb)) WHERE id=$1::uuid AND attempts_count=$2 AND status='running'",[job.id,attempt,now.toISOString(),handled.incompleteCode??null,JSON.stringify(handled.blockedRuleIds)]);
  await audit(client,job,attempt,"continuation_scheduled",now,"bounded_sweep_remaining");return "continued";
}
...
async function fail(client:pg.PoolClient,job:Job,attemptId:string,attempt:number,failureResult:Extract<Handled,{outcome:"failed"}>,now:Date,random:()=>number):Promise<Counter>{
  ...
  if(!retry)await deadLetter(client,job,attemptId,attempt,failureResult.code,summary,failureResult.retryable,now);
...
// prettier-ignore
function failure(code:string):Extract<Handled,{outcome:"failed"}>{return {outcome:"failed",code,retryable:code==="write_unavailable"||code==="revision_conflict"||code==="predecessor_not_projected"||TRANSIENT.has(code)};}
// prettier-ignore
