...
export async function captureFinanceOtaCommissionEvidence(
  ...
  const timezone = getTimezone(input.propertyTimezone.timeZone);
  // prettier-ignore
  if (input.propertyTimezone.source.ownerDomain !== "hotel_catalog" || input.propertyTimezone.source.entityType !== "property_profile" || input.propertyTimezone.source.entityId !== input.propertyId || !/^profile:[1-9]\d*$/.test(input.propertyTimezone.source.revision) || !timezone || timezone.name !== input.propertyTimezone.timeZone || timezone.aliasOf !== null)
