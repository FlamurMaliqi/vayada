  ...
  it("creates one audited expense and rejects mismatched or partial evidence", async () => {
    ...
    await admin.query(
      `UPDATE platform.idempotency_keys SET idempotency_metadata=jsonb_set(idempotency_metadata,'{result,item,unexpected}','true')
       WHERE operation='finance.manual_expense.create' AND property_id=$1 AND correlation_id='correlation-same'`,
      [PROPERTY],
    );
    ...
    const evidence = await admin.query<{ expenses: number; audits: number; keys: number }>(
      `SELECT (SELECT count(*)::int FROM finance.expenses WHERE property_id=$1) AS expenses,
        (SELECT count(*)::int FROM platform.product_audit_events WHERE property_id=$1
          AND NOT (redacted_payload ?| ARRAY['reason','requestedAt','actorOrganizationId'])
          AND private_payload->>'reason'='test' AND audit_metadata @> jsonb_build_object('requestId','request-same','requestedAt','2026-08-10T12:00:00Z','actorOrganizationId',$2::text)) AS audits,
        (SELECT count(*)::int FROM platform.idempotency_keys WHERE property_id=$1) AS keys`,
      [PROPERTY, input.audit.actor.organizationId],
    );
