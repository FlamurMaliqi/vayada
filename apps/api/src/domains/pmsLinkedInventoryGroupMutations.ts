...
export async function putPmsLinkedInventoryGroup(
  ...
  const members = await client.query<{
    ...
    `SELECT id::text AS "roomTypeId", linked_inventory_group_id::text AS "linkedGroupId", active
     FROM pms.room_types WHERE property_id=$1::uuid AND id=ANY($2::uuid[])
     ORDER BY id FOR UPDATE`,
    [command.propertyId, memberIds],
  );
  if (members.rows.length !== memberIds.length || members.rows.some(({ active }) => !active)) {
    return invalidLinkedInventoryGroup(
      "Every linked room type must be active and belong to this property.",
    );
  }
  ...
  const inexactAssignment = await findActiveInexactAssignment(
    client,
    command.propertyId,
    memberIds,
  );
  if (inexactAssignment) {
    return linkedInventoryConflict(
      "linked_inventory_overlap_conflict",
      `Room type ${inexactAssignment.roomTypeId} has an active booking assignment without exact stay dates.`,
    );
  }
...
async function findLinkedInventoryOverlap(
  ...
  const result = await client.query<ConflictRow>(
    `WITH causes AS (
       SELECT receipt.room_type_id,receipt.check_in AS starts_on,receipt.check_out AS ends_on
...
async function findActiveInexactAssignment(
  client: PmsLinkedInventoryGroupClient,
  propertyId: string,
  memberIds: string[],
): Promise<{ roomTypeId: string } | null> {
  const result = await client.query<{ roomTypeId: string }>(
    `SELECT room_type_id::text AS "roomTypeId"
     ...
     WHERE property_id=$1::uuid AND room_type_id=ANY($2::uuid[])
       AND stay_evidence_kind<>'exact'
       AND assignment_status NOT IN ('canceled','released','checked_out')
