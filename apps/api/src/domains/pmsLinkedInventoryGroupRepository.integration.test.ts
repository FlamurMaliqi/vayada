...
describe.skipIf(!URL)("PostgreSQL linked inventory group management", () => {
  ...
  it("creates, replays, replaces, deletes, and rejects overlapping members", async () => {
    ...
    await expect(linkedState()).resolves.toMatchObject({ ariIntents: 2, distributionIntents: 2 });
    await expect(
      repository.replace(
        command("replace", {
          groupId: GROUP,
    ...
    await expect(linkedState()).resolves.toMatchObject({ ariIntents: 5, distributionIntents: 5 });
