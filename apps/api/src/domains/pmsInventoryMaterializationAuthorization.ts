...
import type { PmsInventoryMaterializationAuthorizationPort } from "./pmsInventoryMaterializationRepository.js";

export type PmsInventoryMaterializationAuthorizationPool = {
...
export function createPgPmsInventoryMaterializationAuthorizationPort(options: {
  pool: PmsInventoryMaterializationAuthorizationPool;
}): PmsInventoryMaterializationAuthorizationPort {
  return Object.freeze({
    async authorizeInventoryMaterialization(input) {
      if (
        !UUID_PATTERN.test(input.organizationId) ||
        !UUID_PATTERN.test(input.propertyId) ||
        input.audit.actor.kind !== "user" ||
        !UUID_PATTERN.test(input.audit.actor.userId) ||
        !canonicalIso(input.audit.requestedAt)
      ) {
        return false;
      }
      try {
        const result = await options.pool.query<{ authorized: boolean }>({
          ...
          values: [
            ...
            input.audit.actor.userId.toLowerCase(),
