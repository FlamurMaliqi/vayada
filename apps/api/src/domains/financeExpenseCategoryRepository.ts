...
// prettier-ignore
export type CreateFinanceExpenseCategoryCommand = { commandId: string; idempotencyKey: string;
  propertyId: string; name: string; color: string; sortOrder: number; audit: FinanceCommandAudit };
...
function validate(command: CreateFinanceExpenseCategoryCommand): void {
  ...
  if (
    !exact(command, "commandId idempotencyKey propertyId name color sortOrder audit") ||
