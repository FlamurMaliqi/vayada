...
export type ArchiveFinanceExpenseCategoryCommand = CategoryCommandBase & {
  categoryId: string;
...
export type MutateFinanceExpenseCategoryResult =
  ...
  | {
      status: "conflict";
      reason:
        "revision_conflict" | "already_archived" | "idempotency_key_reused" | "command_in_progress";
    };
...
function validate(command: CreateFinanceExpenseCategoryCommand): void {
  if (!validContext(command) || !validFields(command, true))
...
function validContext(command: CategoryCommandBase): boolean {
...
function storedCategory(value: unknown, created: boolean): FinanceExpenseCategory | null {
  if (
    !record(value) ||
    !uuid(value["id"]) ||
    !(value["systemKey"] === null || trimmed(value["systemKey"], 1, 50)) ||
