...
describe.skipIf(!databaseUrl)("Feature Hub data preservation", () => {
  ...
  it("disables public affiliate access while preserving the complete partner record through reactivation", async () => {
    const snapshot = () =>
      client.query(`SELECT * FROM marketplace.property_affiliates WHERE property_id = $1`, [
        propertyId,
      ]);
    const before = (await snapshot()).rows;
    for (const isActive of [true, false, false, true]) {
      expect((await repository.update(context, propertyId, "affiliates", isActive)).isActive).toBe(
        isActive,
      );
      expect((await repository.list(context, propertyId))[0].isActive).toBe(isActive);
      expect(await resolver.findProfileBySlug(slug)).toMatchObject({
        hotel: { capabilities: { referralCodes: isActive } },
      });
