...
export function CollaborationRequestDetailModal({
      ...
      if (totalFollowers > 0) {
        const weightedEngagement =
          collaboration.creator.platforms.reduce(
            (sum, p) =>
              sum + p.followers * (typeof p.engagementRate === "number" ? p.engagementRate : 0),
            0,
          ) / totalFollowers;
  ...
  const portfolioLink =
    currentUserType === "hotel" ? collaboration.creator?.portfolioLink : undefined;
  ...
  const hasUserAgreed =
    (currentUserType === "hotel" && collaboration.hotelAgreedAt) ||
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-start gap-4 pb-6 border-b border-gray-200">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex-shrink-0 overflow-hidden relative">
                {(currentUserType === "hotel"
                  ? collaboration.creator?.profilePicture
                  : collaboration.hotel?.picture) && !imageError ? (
                  <Image
                    src={
                      (currentUserType === "hotel"
                        ? collaboration.creator?.profilePicture
                        : collaboration.hotel?.picture)!
                    }
                    alt={otherPartyName}
                    fill
                    className="object-cover"
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {collaboration.listingName && (
                      <p className="text-[11px] text-gray-400 font-medium">
                        Applied to:{" "}
                        <span className="text-primary-600">{collaboration.listingName}</span>
                      </p>
                    )}
                  </div>
                )}
            ...
            {currentUserType === "creator" &&
              (collaboration.hotelAbout || collaboration.hotelWebsite) && (
                <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                  ...
                  {collaboration.hotelWebsite && (
                    <a
                      href={
                        collaboration.hotelWebsite.startsWith("http")
                          ? collaboration.hotelWebsite
                          : `https://${collaboration.hotelWebsite}`
                      }
                      target="_blank"
            ...
            <div>
              ...
              {collaboration.travelDateFrom || collaboration.preferredDateFrom ? (
                ...
                <p className="text-gray-700">
                  Preferred Months: {collaboration.preferredMonths.join(", ")}
                </p>
              ) : (
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                  <p className="text-gray-700 font-medium">
                    ...
                    {collaboration.collaborationType === "Free Stay" &&
                      collaboration.freeStayMaxNights &&
                      ` • ${collaboration.freeStayMaxNights} Nights`}
                    {collaboration.collaborationType === "Paid" &&
                      collaboration.paidAmount &&
                      ` • ${getCurrencySymbol(collaboration.currency || "USD")}${Number(collaboration.paidAmount).toLocaleString()}`}
                    {collaboration.collaborationType === "Discount" &&
                      collaboration.discountPercentage &&
                      ` • ${collaboration.discountPercentage}% Off`}
                    {collaboration.collaborationType === "Affiliate" &&
                      collaboration.creatorFee &&
                      ` • ${collaboration.creatorFee}% Commission`}
                  </p>
                </div>
              </div>
            )}
            ...
            {currentUserType === "creator" && collaboration.creatorRequirements && (
              <div className="bg-[#F8FAFC] rounded-2xl p-5 border border-[#E2E8F0]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                  {collaboration.creatorRequirements.platforms &&
                    collaboration.creatorRequirements.platforms.length > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {collaboration.creatorRequirements.platforms.map((p, i) => (
                            ...
                            >
                              {p}
                              {i < (collaboration.creatorRequirements?.platforms.length || 0) - 1
                                ? ", "
                                : ""}
                            </span>
                          ))}
                  ...
                  {collaboration.creatorRequirements.targetCountries &&
                    collaboration.creatorRequirements.targetCountries.length > 0 && (
                      <div>
                        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">
                          Top Countries
                        </p>
                        ...
                        <p className="text-sm font-semibold text-gray-900">
                          {collaboration.creatorRequirements.targetCountries.join(", ")}
                        </p>
                      </div>
                    )}
            ...
            {collaboration.platformDeliverables &&
              collaboration.platformDeliverables.length > 0 && (
                <div>
                    ...
                    {collaboration.platformDeliverables.map((item, index) => (
                      <div key={index} className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                          <div className="text-blue-600">{getPlatformIcon(item.platform)}</div>
                        ...
                        {currentUserType === "creator" ? (
                          ...
                          <div className="ml-7 space-y-1">
                            {item.deliverables.map((deliverable, dIndex) => (
                              <div
                                key={dIndex}
                                className="text-sm text-gray-600 bg-gray-50 px-3 py-1.5 rounded-lg inline-block w-fit"
                              >
                                {deliverable.quantity}x {deliverable.type}
                              </div>
                            ))}
                            ...
                            <div className="space-y-2">
                              {platform.topAgeGroups.slice(0, 3).map((ageGroup, idx) => (
                                <div key={idx} className="flex items-center justify-between">
                                  ...
                                  {typeof ageGroup.percentage === "number" &&
                ...
                <div className="border border-gray-200 rounded-xl bg-white shadow-sm overflow-hidden">
                  {collaboration.creator.rating.totalReviews > 0 ? (
                    <>
                      ...
                      {collaboration.creator.rating.reviews &&
                        collaboration.creator.rating.reviews.length > 0 && (
                ...
                <>
                  {hasUserAgreed ? (
                    <div className="text-gray-400 text-sm font-medium flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-lg border border-gray-100">
                      <ArrowPathIcon className="w-4 h-4 animate-spin-slow" /> Waiting for{" "}
                      {otherPartyName}...
                    </div>
                  ) : (
                    <Button
