...
export default function LinkedInventoryGroupsPanel({
  ...
  const save = async () => {
    ...
    try {
      const saved = draft.groupId
        ? await linkedInventoryGroupsService.update(
            draft.groupId,
            draft.name.trim(),
            draft.memberRoomTypeIds,
          )
        : await linkedInventoryGroupsService.create(draft.name.trim(), draft.memberRoomTypeIds);
      setDraft(null);
      onChange((current) =>
  ...
  const remove = async (group: LinkedInventoryGroup) => {
    ...
    setError("");
    try {
      await linkedInventoryGroupsService.delete(group.groupId);
