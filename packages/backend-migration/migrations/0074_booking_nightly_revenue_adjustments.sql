...
ALTER TABLE booking.nightly_revenue_evidence
  ADD CONSTRAINT chk_booking_nightly_revenue_evidence_enums CHECK (
    economic_event IN ('room_night', 'room_night_reversal', 'occupancy_adjustment', 'retained_charge', 'refund', 'correction')
  ...
  ADD CONSTRAINT chk_booking_nightly_revenue_evidence_event CHECK (
    ...
    OR (economic_event = 'occupancy_adjustment' AND recognized_on >= stay_date
      AND occupied_room_nights IN (-1, 1) AND corrects_evidence_id IS NOT NULL
...
BEGIN
  ...
  IF NEW.corrects_evidence_id IS DISTINCT FROM current_tip OR NOT
    ...
    RAISE EXCEPTION 'occupancy adjustment must toggle the current room-night tip'
      USING ERRCODE = '23514', CONSTRAINT = 'chk_booking_nightly_revenue_evidence_occupancy_transition';
  END IF;
  RETURN NEW;
END;
$$;
CREATE TRIGGER trg_booking_nightly_revenue_evidence_validate_occupancy BEFORE INSERT
ON booking.nightly_revenue_evidence FOR EACH ROW EXECUTE FUNCTION booking.validate_nightly_revenue_occupancy_change();
