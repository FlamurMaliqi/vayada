-- Migration: 0112_pms_inventory_receipt_update_handoff
...
BEGIN
  ...
  THEN
    IF explicit_receipt_text IS NOT NULL
      AND booking_receipt_text IS DISTINCT FROM explicit_receipt_text
    THEN
  ...
  IF receipt_row.lifecycle_state = 'reserved' AND (
    ...
    OR booking_receipt_text IS DISTINCT FROM explicit_receipt_text
  ) THEN
...
DROP TRIGGER trg_pms_direct_booking_inventory_receipt_handoff
