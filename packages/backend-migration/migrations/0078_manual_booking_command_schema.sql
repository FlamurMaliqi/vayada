...
ALTER TABLE pms.operational_booking_assignments
  ...
  ADD CONSTRAINT chk_pms_operational_assignments_position
    CHECK (position >= 1 AND (stay_evidence_kind = 'summary_only' OR position <= 20)),
  ...
  ADD CONSTRAINT chk_pms_operational_assignments_stay_evidence
    CHECK (
      ...
      (stay_evidence_kind = 'exact'
        ...
        AND isfinite(check_in) AND isfinite(check_out)
...
BEGIN
  IF NEW.stay_evidence_kind = 'exact' AND NEW.room_id IS NULL AND
    (TG_OP = 'INSERT' OR NEW.stay_evidence_kind IS DISTINCT FROM OLD.stay_evidence_kind) THEN
