-- Migration: 0038_require_creator_profile_identity_photo
-- Owner: domain-marketplace
--
-- Creator profile photos are mandatory. Keep identity-photo reuse constrained
-- to the profile owner's currently selected, managed public image.

DROP FUNCTION marketplace.creator_profile_is_complete(UUID, UUID, BOOLEAN);
    ...
    WHERE profile.id = p_creator_profile_id
      ...
      AND EXISTS (
        ...
        WHERE media.id::text = profile.profile_metadata ->> 'profilePictureMediaObjectId'
          ...
          AND (
            (
              media.resource_product = 'marketplace'
            ...
            OR (
              ...
              AND EXISTS (
                SELECT 1
                FROM identity.users AS owner
                WHERE owner.id = profile.owner_user_id
                  AND owner.profile_picture_media_object_id = media.id::text
                  AND owner.profile_picture_url = profile.profile_picture_url
              )
            )
          )
      )
