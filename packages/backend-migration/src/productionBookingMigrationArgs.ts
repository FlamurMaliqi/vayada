import type { ProductionBookingMigrationMode } from "./productionBookingMigration.js";

export type ProductionBookingMigrationArgs = {
  connectionString: string;
  sourceRunId: string;
  mode: ProductionBookingMigrationMode;
};

export function parseProductionBookingMigrationArgs(
  argv: string[],
  environment: Record<string, string | undefined> = process.env,
): ProductionBookingMigrationArgs {
  const args = argv.slice(2);
  let connectionString = environment["TARGET_DATABASE_URL"] ?? "";
  let sourceRunId = "";
  let confirm = "";
  let explicitMode: ProductionBookingMigrationMode | undefined;
  ...
  const expected = `production-booking:${sourceRunId}`;
