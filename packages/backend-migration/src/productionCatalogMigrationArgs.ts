import type { ProductionCatalogMigrationMode } from "./productionCatalogMigration.js";

export type ProductionCatalogMigrationArgs = {
  connectionString: string;
  sourceRunId: string;
  mode: ProductionCatalogMigrationMode;
};

export function parseProductionCatalogMigrationArgs(
  argv: string[],
  environment: Record<string, string | undefined> = process.env,
): ProductionCatalogMigrationArgs {
  const args = argv.slice(2);
  let connectionString = environment["TARGET_DATABASE_URL"] ?? "";
  let sourceRunId = "";
  let confirm = "";
  let explicitMode: ProductionCatalogMigrationMode | undefined;
  ...
  const expected = `production-catalog:${sourceRunId}`;
