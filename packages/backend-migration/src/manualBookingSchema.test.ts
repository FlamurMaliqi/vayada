...
const migration = await readFile(
  join(import.meta.dirname, "../migrations/0078_manual_booking_command_schema.sql"),
  "utf8",
);
...
describe.skipIf(!TEST_DATABASE_URL)("manual-booking target schema (PostgreSQL)", () => {
  ...
  it("keeps migrated assignments summary-only and old reads compatible", async () => {
    await client.query(
      "UPDATE pms.operational_booking_assignments SET assignment_status='released' WHERE guest_booking_id=$1",
      [SUMMARY_BOOKING],
    );
    const result = await client.query(
      `SELECT array_agg(position ORDER BY position) positions,
        bool_and(stay_evidence_kind='summary_only' AND check_in IS NULL AND adults IS NULL) compatible
       FROM pms.operational_booking_assignments WHERE guest_booking_id=$1`,
      [SUMMARY_BOOKING],
    );
    expect(result.rows[0]).toEqual({ positions: [21, 23], compatible: true });
  });
  ...
  it("stores heterogeneous exact stays without JSON authority", async () => {
    ...
    const result = await client.query<{ stays: string }>(
      `SELECT string_agg(position||':'||room_type_id||':'||COALESCE(rate_plan_id::text,'null')||':'||check_in||':'||check_out||':'||adults||':'||children,';' ORDER BY position) stays
       FROM pms.operational_booking_assignments WHERE guest_booking_id=$1`,
      [EXACT_BOOKING],
    );
    expect(result.rows[0]!.stays).toBe(
      `1:${TYPE_A}:${PLAN_A}:2026-09-01:2026-09-04:2:0;2:${TYPE_B}:null:2026-09-03:2026-09-07:1:2`,
    );
    await client.query(
      "UPDATE pms.operational_booking_assignments SET room_id=NULL,assignment_status='released' WHERE guest_booking_id=$1",
      [EXACT_BOOKING],
    );
  });
...
async function createPredecessorSchema(client: pg.Client): Promise<void> {
  await client.query(`
    ...
    VALUES (gen_random_uuid(), '${PROPERTY}', '${SUMMARY_BOOKING}', '${TYPE_A}', '${ROOM_A}', 21, 'assigned', 'manual'),
      (gen_random_uuid(), '${PROPERTY}', '${SUMMARY_BOOKING}', '${TYPE_A}', '${ROOM_A}', 23, 'assigned', 'manual');
  `);
}
