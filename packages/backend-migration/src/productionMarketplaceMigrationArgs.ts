import type { ProductionMarketplaceMigrationMode } from "./productionMarketplaceMigration.js";

export type ProductionMarketplaceMigrationArgs = {
...
export function parseProductionMarketplaceMigrationArgs(
  argv: string[],
  environment: Record<string, string | undefined> = process.env,
): ProductionMarketplaceMigrationArgs {
  const args = argv.slice(2);
  let connectionString = environment["TARGET_DATABASE_URL"] ?? "";
  let sourceRunId = "";
  let confirm = "";
  let explicitMode: ProductionMarketplaceMigrationMode | undefined;
  ...
  const expected = `production-marketplace:${sourceRunId}`;
