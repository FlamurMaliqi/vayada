...
_Updated 2026-08-19 after retiring the public-profile legacy-database adapter._
...
- The only accepted legacy product database URL left in `ApiConfig` is
  `BOOKING_DATABASE_URL`. It now feeds only the Booking settings reader/writer
  described below.
...
- `API_RUNTIME=next` forbids `BOOKING_DATABASE_URL`, always resolves Booking Web
  hosts through the selected target profile repository, and requires target
  public bookability, Booking settings, Finance, and checkout sources. The
  target reservation route has no remaining source selector.
...
Remove the remaining legacy Booking DB support next:

1. Remove `createPgBookingSettingsReadRepository`,
