...
_Updated 2026-08-19 after removing the public-bookability compatibility selector._
...
- `API_RUNTIME=next` always resolves Booking Web hosts, quotes, and calendars
  through target repositories and requires target Finance, PMS operations, and
  checkout sources. Booking settings, reservations, and public bookability no
...
Remove `BOOKING_CHECKOUT_COMMAND_SOURCE=legacy_proxy`, the remaining no-service
compatibility selector, in its own route-focused change. The domain-resolution
selector was removed with the profile reader.
