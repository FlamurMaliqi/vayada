...
The row-level `Status` values below are the historical VAY-669 planning
snapshot, not the current implementation baseline. Registered fixture
transforms and parity checks now cover active Hotel Catalog, Booking, Finance,
PMS, Marketplace, Distribution, and Platform Media paths. Treat those cases as
**existing work to extend**, even where an older row says `DDL only` or
`not started`.
...
| `login_audit_log`           | Jobs/events/audit | `platform.product_audit_events`                                                                                 | migrate              | done (DDL only)        | Audit event count; actor/correlation metadata parity                                                  | PII          |
...
| `channex_webhook_events`                                      | Jobs/events/audit                                       | `platform.external_webhook_events`                                                                                                                                                                                             | migrate     | done (DDL only)            | Webhook delivery log count; pipeline health parity                                                                             | private      |
...
| PMS-local `platform.media_objects`, `platform.media_variants` | Platform Media                                          | Canonical `platform.media_objects`, `platform.media_variants`                                                                                                                                                                  | reconcile   | done (source + target DDL) | Media-object and variant counts; lifecycle/visibility parity; attachment foreign-reference integrity                           | conditional  |
...
| `property_module_activations`                                 | Finance + typed product readiness + Ask Intelligence    | `finance.billing_entitlements`; producer readiness projections consumed by `adaptive_property_setup_read_model`; `intelligence.setup_completeness_snapshots`                                                                   | transform   | not started                | Module activation parity against entitlements plus producer readiness; no canonical `property_setup_status` table              | private      |
...
| 7     | `intelligence`                 | Done in VAY-677                                  | DDL added for `metric_definitions`, `metric_snapshot_runs`, `setup_completeness_snapshots`, `ai_evidence_catalog`, `ask_conversations`, `ask_runs`, `ask_tool_calls`, and `ask_answer_audits`. Source-to-target transforms, snapshot generation jobs, parity fixtures, and runtime Ask agents remain open.                                                                                                                                                                                                | Finance, booking, PMS, and marketplace read models needed as evidence inputs; Ask Intelligence evidence contract done                          |
...
Ask Intelligence target DDL landed in VAY-677 via
`packages/backend-migration/migrations/0011_intelligence.sql`. The DDL adds the
target tables for metric definitions, metric/setup snapshots, evidence catalog,
conversations, runs, tool calls, and answer audits. It does not add snapshot
generation jobs, source-to-target transforms, parity fixtures, or runtime Ask
agent behavior.
