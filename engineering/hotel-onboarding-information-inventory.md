...
Existing `profile.default_locale` and `profile.supported_locales` storage
belongs to that future translation design and is not part of first-run
onboarding.
...
Language, category, star rating, location, and contact information are not
asked again.
...
| Field ID                                     | Treatment | Publication effect            | Target owner and current contract                               |
| -------------------------------------------- | --------- | ----------------------------- | --------------------------------------------------------------- |
| `marketplace.preferences.compensation_types` | Required  | Blocks Marketplace submission | `marketplace.hotel_collaboration_preferences`; write API exists |
| `marketplace.preferences.content_platforms`  | Required  | Blocks Marketplace submission | Same preference contract                                        |
| `marketplace.preferences.content_types`      | Required  | Blocks Marketplace submission | Same preference contract                                        |
| `marketplace.preferences.availability`       | Required  | Blocks Marketplace submission | Same preference contract                                        |
...
- Back, Review setup, and Exit setup preserve dirty pricing fields in the
  resumable onboarding draft. Review and launch later builds and atomically
  publishes a Booking content revision from the selected room, pricing, policy,
  and page-design versions. It records the initial fully materialized calendar
...
| `policy.check_in_time`, `policy.check_out_time`                                        | Required local times                                           | Block Booking publication                  | Hotel Catalog policy values; the current broad Booking write can overwrite unrelated policy data                        |
...
- Step 7 may write only the canonical check-in and check-out values through
  the Hotel Catalog owner boundary. It must not submit a broad property-policy
  replacement.
...
- The backend orchestrator validates the source revisions and derived
  currency, saves the Booking-owned choices, writes check-in/out through Hotel
  Catalog, generates the policy bundle, stores confirmation evidence, and
...
- one orchestration boundary that writes Booking-owned choices and only the
  allowed Hotel Catalog check-in/out fields without broad cross-domain table
  writes;
...
- independent Marketplace and Booking activation pointers so partial success
...
- Preserve the Booking/PMS boundary. Booking owns guest-facing settings,
  projection, checkout, and publication. PMS owns room types, physical units,
  rates, calendar configuration, inventory, and operational reservations.
...
| [ONB-02A](https://linear.app/vayadacom/issue/VAY-1048/define-onboarding-revision-and-lifecycle-contracts)  | Does every producer use one typed source-manifest/revision vocabulary, one readiness-port result, and explicit immutable submission/publication revision plus active-pointer contracts?                                                                                 | ONB-00, ONB-02                                                  | A deterministic manifest/readiness hash and structured product/group/step/entity/blocker result are fixture-tested; Marketplace and Booking lifecycles are independent; Booking content revision/active pointer and live-ARI watermark are distinct; no raw cross-domain join is needed. |
...
| ONB-06                                                                                                     | Does the preserved prerequisite flow collect one property logo while removing the hotel-manager personal-photo requirement?                                                                                                                                             | ONB-05                                                          | Hotel accounts finish the preserved flow with a property-owned logo; creator accounts still require their personal photo; manager menus use initials; the logo is never stored on the user.                                                                                              |
| ONB-07                                                                                                     | Can Step 1 save short summary, optional amenities, and canonical media while preserving incomplete input and deterministic slug/alt-text generation?                                                                                                                    | ONB-01, ONB-05                                                  | Canonical writes use expected profile revision; draft-only partial input resumes; explicit empty amenities are valid; generated fields are not requested from the owner; Step 1 browser flow passes.                                                                                     |
| ONB-08                                                                                                     | Can Step 2 reuse the approved complete Marketplace preference contract while drafts represent unanswered and partial groups safely?                                                                                                                                     | ONB-01, ONB-07                                                  | Canonical `PUT` stays strict; incomplete draft does not create false `year_round` readiness; all four groups and selected-month validation pass with route-policy denial coverage.                                                                                                       |
...
| ONB-28                                                                                                     | Can Booking consume typed producer ports to build and validate a complete immutable public projection before activation, then atomically move the active content pointer through an idempotent, expected-manifest publication operation?                                | ONB-02A, ONB-10, ONB-13, ONB-17, ONB-20, ONB-22, ONB-24, ONB-27 | Initial build needs no already-public profile; failed/stale builds leave the old page live; missing content/ARI watermark fails closed; readiness loss suppresses unsafe offers immediately while gain needs republish.                                                                  |
...
| ONB-30                                                                                                     | Is the complete replacement safe to expose and cut over?                                                                                                                                                                                                                | ONB-00–29, including ONB-02A, ONB-04A, ONB-25A, and ONB-28A     | Target migrations/parity, authorization denial suites, build/typecheck, browser/mobile/accessibility, public/private audit, staging rehearsal, and rollback gates pass; admin invite dependencies are migrated and retired wizard exports/routes are removed after the switch.           |
...
| Finance/payments       | ONB-24 → ONB-25 → ONB-25A → ONB-26          | Provider connection may precede card execution; only charge-ready Stripe becomes eligible for explicit publication |
