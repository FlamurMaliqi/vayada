...
| `organization_memberships`        | Identity/auth            | Existing owner/admin user associations, WorkOS memberships, migration transforms from `users.type` and `is_superadmin`.         | Membership-local role and delegation origin; `users.type` and `is_superadmin` are transform inputs only.                                                                                              |
| `membership_property_assignments` | Identity/auth            | VAY-1085 staff access contract; no legacy authorization source.                                                                 | Explicit membership scope keyed to canonical hotel-catalog properties.                                                                                                                                |
| `membership_delegations`          | Identity/auth            | VAY-1321 external-owner delegation contract; no legacy authorization source.                                                    | Same-organization external-owner-to-staff edge used as a live permission and property ceiling.                                                                                                        |
...
| `guest_bookings`                    | Booking/checkout            | PMS `bookings`, PMS `booking_rooms`, PMS booking drafts when materialized, booking public references.        | Guest-facing lifecycle, aggregate stay summaries, and expected-payment-method intent behind a typed Booking owner port.                    |
| `nightly_revenue_evidence`          | Booking/checkout            | New immutable direct, OTA, manual, and migrated per-night economic evidence.                                 | Finance-safe view exposes room revenue plus append-only reversal, retained-charge, refund, and correction facts.                           |
...
| `direct_booking_summary_read_model` | Booking/checkout read model | `guest_bookings`, PMS operational assignments, payments, property catalog.                                   | Consumed by PMS and finance through permissioned views.                                                                                    |
...
| `linked_inventory_groups`           | PMS operations            | VAY-1338 linked-inventory configuration; no legacy source.                                                  | Groups mutually exclusive room-type configurations for one property; membership is stored on `room_types`.                                                        |
...
| `room_blocks`                       | PMS operations            | PMS `room_blocks`; VAY-1338 derived linked blocks.                                                         | Manual operational blockers plus protected, causally linked booking/manual cross-blocks.                                                                           |
| `operational_booking_assignments`   | PMS operations            | PMS booking room assignments, `booking_rooms`, room IDs, auto-rearrange settings.                          | Owns per-stay room/type/rate-plan, dates, occupancy, and position; migrated rows remain explicitly `summary_only`.                                                 |
...
| `pms_operations_summary_read_model` | PMS operations read model | Rooms, rates, assignments, channel status, booking summaries.                                              | Consumed by admin dashboards through permissioned views.                                                                                                           |
...
| `affiliate_payout_payment_evidence` and item rows    | Finance                            | New Platform Admin manual/bank-transfer payment evidence.                                                   | Append-only payout snapshots, external references, and operator evidence.                                                                        |
...
| `expense_categories`                                 | Finance                            | New property-scoped Finance category registry.                                                              | Closed system keys with customizable presentation and archive-safe references.                                                                   |
| `recurring_expense_rules`                            | Finance                            | New recurring expense templates.                                                                            | Stores schedules only; generation creates immutable expense evidence.                                                                            |
| `expenses`                                           | Finance                            | New manual and generated expense evidence.                                                                  | Correction-safe ledger; supplier document numbers are external evidence, not Vayada invoices.                                                    |
| `folios`                                             | Finance                            | New property-scoped operational folio identities.                                                           | Opaque IDs group booking charges without representing official invoices.                                                                         |
| `folio_revisions`                                    | Finance                            | New immutable folio snapshots.                                                                              | Draft, ready, and archived operational evidence; superseded is derived.                                                                          |
| `folio_lines`                                        | Finance                            | New normalized operational folio charges.                                                                   | Immutable source-linked lines with database-derived totals.                                                                                      |
| `folio_payment_references`                           | Finance                            | New references to canonical Finance payments.                                                               | Snapshot amounts only; no provider state or accounting allocation.                                                                               |
| `finance_visibility_read_model`                      | Finance read model                 | Payments, payouts, commissions, entitlements, selected resource links.                                      | Admin dashboards consume only with explicit finance permission.                                                                                  |
...
> Historical ownership record only. Migration `0090` removes this schema, its
...
> boundaries rather than reviving these tables.
...
| PMS property module activations and setup/checklist/dashboard status fields                | Finance, Hotel setup read model                      | `billing_entitlements` and typed product readiness ports consumed by `adaptive_property_setup_read_model`; no canonical `property_setup_status` table.                                         |
...
| Future hotel employee agent (not implemented)      | Unassigned; design in VAY-1091  | No current access contract. VAY-1091 must define new ownership, authorization, tool, and storage boundaries.                                                                   |
